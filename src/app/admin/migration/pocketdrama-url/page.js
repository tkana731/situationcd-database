...
export default function PocketdramaUrlMigrationPage() {
    ...
    const handleMigration = async () => {
        ...
        try {
            ...
            addLog(`Firestoreから${allProducts.length}件の製品データを取得しました`);
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                const matchCandidates = allProducts
                    .map(product => ({
                        product,
                ...
                // 既にポケドラURLが設定されている場合は上書きするか確認
                if (matchedProduct.pocketdramaUrl && matchedProduct.pocketdramaUrl !== '') {
                    // 同じURLなら何もしない
                    if (matchedProduct.pocketdramaUrl === pocketdramaUrl) {
                        addLog(`行 ${i + 2}: 製品 "${matchedProduct.title}" のポケドラURLは既に同じ値が設定されています。スキップします。`);
                        stats.alreadySet++;
                        continue;
                    }
                }
            ...
            <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
                <div className="flex">
                    <div className="ml-3">
                        ...
                        <p className="text-sm text-blue-700 mt-2">
                            処理は自動的に実行され、マッチングされた製品のポケドラURLが更新されます。
                        </p>
                    </div>
                </div>
            </div>
